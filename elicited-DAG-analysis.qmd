...
``` {r}
sim_signal_to_noise_ratios <- list("idyllic" = 5, "typical" = 1, "noisy" = 0.2)

tibble(Scenario = names(sim_signal_to_noise_ratios),
       `Signal:Noise` = as.numeric(sim_signal_to_noise_ratios)) 
```

Here we see that in a "typical" scenario we assume that each path coefficient is `r sim_signal_to_noise_ratios[["typical"]]` times stronger than the noise. For simplicity we set each path coefficient to 0.5 and so in this case the variance in the noise for each node is $0.5^2$/`r sim_signal_to_noise_ratios[["typical"]]`. NEED TO COMPARE THESE CHOICES TO TYPICAL WAYS THESE THINGS ARE TESTED.
...
nodes <- all_nodes_with_metrics |> 
    filter(Model == model) |> 
    select(name, node_group, node_type, measurability) |> 
...
d.sim.logistic <- simulateLogistic(new_dag, b.default = 1, N = 1000)
...
# noise is variance 1
# signal variance is b^2

d.noisy <- simulateSEM(new_dag, b.default = 0.1, eps = 1, standardized = F, N = 1000)
d.typical <- simulateSEM(new_dag, b.default = 0.4, eps = 1, standardized = F,N = 1000)
d.ideal <- simulateSEM(new_dag, b.default = 0.9, eps = 1, standardized = F,N = 1000)

plot_parent_child <- function(d) {
    fit <- lm(Child ~ Parent, 
              data = d |> 
                  mutate(Parent = CD, Child = CD_p))
...
plot_parent_child(d.noisy)
plot_parent_child(d.typical)
plot_parent_child(d.ideal)


# The 't value' tells the clearest story here
summary(lm(CD_p ~ CD, data = d.noisy))
summary(lm(CD_p ~ CD, data = d.typical))
summary(lm(CD_p ~ CD, data = d.ideal))
...
net.full = bnlearn::hc(d.sim.sem)
net.dw = bnlearn::hc(d.sim.sem[,node_list_dw])
net.dwo = bnlearn::hc(d.sim.sem[,node_list_dwo])
net.dwoop = bnlearn::hc(d.ideal[,node_list_dwoop])

bn.full = bnlearn::bn.fit(net.full, d.sim.sem)
bn.dw = bnlearn::bn.fit(net.dw, d.sim.sem[,node_list_dw])
bn.dwo = bnlearn::bn.fit(net.dwo, d.sim.sem[,node_list_dwo])
bn.dwoop = bnlearn::bn.fit(net.dwoop, d.sim.sem[,node_list_dwoop])
