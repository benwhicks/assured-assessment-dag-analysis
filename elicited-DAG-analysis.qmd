...
edges_with_metrics |> 
    count(switched) |> 
    mutate(n = if_else(switched == "aligned", n, n/2)) |> # contested are counted twice due to edge reversal
    mutate(p = n / sum(n))
...
# keep working through the ?bnlearn helper. The causal query might be a good metric ?cpquery


# shared_nodes <- intersect(
#   bnlearn::nodes(bn.full), 
#   bnlearn::nodes(bn.dwoop)
# )
...
# 
# library(bnlearn)
# # Query both - e.g. probability Grade is above some threshold
# # given an intervention on an observable node
# # I also want to query the original DAG. How well do these represent that
# # Ground truth from simulation
# d.typical |> 
#   filter(CD > 1) |> 
#   summarise(p_truth = mean(Grade > 1))
# 
# cpquery(bn.full, 
#         event = (Grade > 1), 
#         evidence = (CD > 1),
...
Workflow:
- Use adjustmentSets to find valid adjustment sets for each exposure-outcome pair (will need to be in both models, or all, or some??)
- Use the simulated data with those adjustment sets as your ground truth causal estimates
- Compare cpquery results against those — the gap tells you how much the structure learning is distorting causal conclusions vs just conditional ones

Maybe the adjustment sets should be for:
- Knowledge -> Grade
- AI use -> Grade
- AI use -> Knowledge

Or perhaps just the first, and we definitely look at AI use as one of the conditional probabilities? Seems closest to the paper problem.

Ok, how about:
- Estimate P(Grade|do(S.Kn)) in each: use the suggested adjustment sets. Maybe a Bayesian approach to get uncertainty (later if runtime allows)
