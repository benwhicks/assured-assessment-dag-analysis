...
``` {r}
sim_signal_strength <- list("idyllic" = 0.9, "typical" = 0.5, "noisy" = 0.1)
...
bn_fit_model_list <- function(m_proxy_list, b) {
    
    # getting simulated data, and subsets
    d.sim.sem <- simSEM(m_proxy_list$dag, b.default = b)
    ...
    bn.dwo =   bnlearn::bn.fit(bnlearn::hc(d.dw), d.dw)
    bn.dwoop = bnlearn::bn.fit(bnlearn::hc(d.dw), d.dw)
...
s1.m <- m_build_model_with_proxy_nodes(m = "S1")
s1.bn.fits <- bn_fit_model_list(s1.m, b = sim_signal_strength[["typical"]])

model_list <- s1.m
bn_list <- s1.bn.fits
run_comparisons_on_bns <- function(model_list, bn_list,
                                   dag, exposure = "S.Kn", outcome = "Grade") {
    orig_adjSets <- adjustmentSets(dag, exposure = exposure, outcome = outcome, type = "all")
    orig_adjSets_canonical <- adjustmentSets(dag, exposure = exposure, outcome = outcome, type = "minimal")
}
...
bnlearn::compare(dagitty_to_bn(dag_s1), bnlearn::bn.net(bn_list$bn.full))

# OR just use adjustment sets...

adjustmentSets(dag_s1, exposure = "S.Kn", outcome = "Grade")
