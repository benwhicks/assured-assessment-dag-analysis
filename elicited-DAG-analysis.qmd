...
p_time_and_representaion <-  
    ...
    mutate(
        ...
        y_validity = GCM.validity * 10 + 20 # rescale to 30, 40, 50, 60 to match units on x axis, for the 
    ) |> 
    ...
    coord_fixed(ratio = 1) +
    annotate("text", x = 39, y = 41, label = "Proportion of nodes visible\nin data warehouse", 
             size = 2.5, color = hue2) +
    annotate("segment", arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
             x = 38.5, xend = 38.6, y = 42.2, yend = 43.5, 
             color = hue2) +
    annotate("text", x = 33, y = 45, 
             label = "Proportion of\nobservable nodes", 
             size = 2.5, 
             color = adjustcolor(hue2, alpha.f = 0.8)) +
    annotate("segment", arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
             x = 36, xend = 37.5, y = 45, yend = 45, alpha = 0.8,
             color = hue2) +
    annotate("text", x = 44, y = 47.7, 
             label = "Proportion of potentially observable nodes", 
             size = 2.5, color = adjustcolor(hue2, alpha.f = 0.6)) +
    annotate("segment", arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
             x = 40, xend = 39.1, y = 47, yend = 46.2, alpha = 0.6,
             color = hue2) +
...
p_time_and_representaion_old <- 
    gcm_metrics |> 
    # mutate(num_dur = as.numeric(Modelling.duration, unit = "mins")) |> 
    mutate(acyclic = if_else(acyclic, "Yes, DAG", "No, contained cycles")) |> 
...
geom_arc(aes(
    x0 = num_dur, 
    y0 = GCM.validity,
    r = 0.2,                          # radius of the ring
    start = 0,                     # start at top (12 o'clock),
    end = 2*pi*measurability   # arc length based on proportion
  ),     
  linewidth = 2, color = hue2) +
    # geom_arc(aes(r = 0.3, x0 = Modelling.duration, y0 = GCM.validity,
    #            start = 0, end = measurability * 2 * pi),
