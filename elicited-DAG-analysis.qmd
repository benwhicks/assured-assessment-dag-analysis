...
library(softImpute)
fit <- softImpute(D_with_NA, rank.max = 4, lambda = 0)
D_filled <- complete(D_with_NA, fit)

compute_dist_matrix <- function(
        model_list, # list of models
        .df, # function that computes distance between two models
        symmetric = FALSE,
        impute_undefined = TRUE) {
    n  <- length(model_list)
    nm <- names(model_list) 
    ...
    if (impute_undefined) {
        ...
        if (any(is.nan(D))) {
            ...
            fit <- softImpute(D, rank.max = 4, lambda = 0)
        D <- complete(D, fit)
        }
        if (any(is.nan()))
        fitSym <- softImpute(Dsym, rank.max = 4, lambda = 0)
...
plotting_Ln_between_distances <- function(D, 
                                      Level = "L1") {
    D_sym <- (D + t(D)) / 2
    nm <- D |> colnames()
    # --- cluster on the symmetric matrix ---
    hc  <- hclust(as.dist(D_sym), method = "average")
    ord <- hc$order
    lev <- nm[ord]  

    # --- dendrogram panel (rotated to sit above the heatmap) ---
    dend <- dendro_data(hc)
    ...
    p_dend <- ggplot() +
        ...
        scale_y_discrete(labels = fmt_lab) +
        theme_minimal() +
        coord_flip() +
        theme(
...
L1_dist <- function(m_i, m_j, mode = "soft") {
  fuzzyL1 <- gcm_fuzzy_L1_consistency(
      gX = m_i, gY = m_j)
  if (mode == "soft") {
      return(1 - fuzzyL1$L1soft)
  } else {
      return(1 - fuzzyL1$L1hard)
  }
}
...
plots_L1_similarities <- plotting_Ln_between_distances(
    L1_between_model_metrics$D, Level = "L1")

```

``` {r}
plots_L1_similarities$p_combined
```
...
plots_L2_similarities <- plotting_Ln_between_distances(
    ...
    L2_between_model_metrics$Dsym,
    Level = "L2"
)

plots_L2_similarities$p_combined
```
...
``` {r}
p1 <- g_plot_fine(tdags$M1, weight_nodes = FALSE)
p2 <- g_plot_fine(tdags$M2, weight_nodes = FALSE)
p3 <- g_plot_fine(tdags$M3, weight_nodes = FALSE)
p4 <- g_plot_fine(tdags$M4, weight_nodes = FALSE)
p5 <- g_plot_fine(tdags$M5, weight_nodes = FALSE)
p6 <- g_plot_fine(tdags$M6, weight_nodes = FALSE)
p7 <- g_plot_fine(tdags$M7, weight_nodes = FALSE)
p8 <- g_plot_fine(tdags$M8, weight_nodes = FALSE)
...
# Error here - only using 8 models on D - something wrong in algorithm
L1_between_model_and_simulation_metrics <- 
    compute_dist_matrix(all.cluster.dags, L1_dist, symmetric = TRUE)

# Filling in errors with max distance
# L1_between_model_and_simulation_metrics$D[is.nan(L1_between_model_and_simulation_metrics$D)] <- 1

plots_L1_similarities_full <- plotting_Ln_between_distances(
    L1_between_model_and_simulation_metrics$D, Level = "L1")
...
L2_between_model_metrics_full$D <- 
    compute_dist_matrix(all.cluster.dags, L2_dist, symmetric = FALSE)
L2_between_model_metrics_full$D[is.nan(L2_between_model_metrics_full$D)] <- 1
L2_between_model_metrics_full$Dsym[is.nan(L2_between_model_metrics_full$Dsym)] <- 1
```

``` {r}
plots_L2_similarities_full <- plotting_Ln_between_distances(
    L2_between_model_metrics_full$D, 
    L2_between_model_metrics_full$Dsym,
    Level = "L2"
)
...
plots_L12_mean_consistency_all <- 
    plotting_Ln_between_distances(
        (L1_between_model_and_simulation_metrics$D + L2_between_model_metrics_full$Dsym) / 2,
        Level = "L1-2"
    )


plots_L12_mean_consistency_all$p_denro
```