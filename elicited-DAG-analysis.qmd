...
We will use $\mathcal{CURVEY}$ uppercase script for sets across all the models, $\mathbf{BOLD}$ uppercase for sets, and $UPPER$ case standard for instances in the models. Lowercase like $v$ will be reserved for imagining actual values (so a node taking on a particular value).  

The models are indexed as $\mathcal{G}=\{G_1,G_2,\dots,G_8 \}$, and each model $G_k$ comprises of a set of vertices $\mathbf{V}_{G_k}$ and edges $\mathbf{E}_{G_k}$, so $G_k=(\mathbf{V}_{G_k}, \mathbf{E}_{G_k})$, with the set of all nodes being $\mathcal{V}$ edges being $\mathcal{E}$. $E_{ij}=V_i \rightarrow V_j$ indicates an edge between two nodes. If two nodes are connected through a mediated path, such as $V_i \rightarrow \dots \rightarrow V_j$ then we write $\mathrm{De}_{ij}=V_i \rightsquigarrow V_j$ or $V_i \in \mathrm{an}(V_j)$. To specify model context for ancestors, descendants and mediated parts we subscript the model: $V_i \in \mathrm{an}_{G_k}(V_j)$ or $V_i \rightsquigarrow _{G_k} V_j$. Nodes $V_i$ are indexed over all models (so across $\mathcal{V}$), but if we want to talk about $V_i$ as it stands in $G_k$ we would use $V_{i:G_k}$, and likewise for an edge $E_{i:G_k}$ residing in the context of model $G_k$.
...
plot_bnlearn_coarse_comparison(s_estimates = simulations_ng_cluster_models$M1$estimates) + 
