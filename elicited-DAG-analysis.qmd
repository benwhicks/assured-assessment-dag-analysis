...
Now we examine the feasibility of learning these models from data. A typical counter to going to the effort of eliciting these models from domain experts is that they can be learned from the data alone. However, we have seen here that much of the data is not present in the institutions ecosystem, with the nodes being deemed inherintly latent (`r round(100*(sum(all_nodes_with_metrics$measurability == "L") / nrow(all_nodes_with_metrics)), 1)`%), potentially observable (`r round(100*(sum(all_nodes_with_metrics$measurability == "PO") / nrow(all_nodes_with_metrics)), 1)`%), observable but not currently in the data (`r round(100*(sum(all_nodes_with_metrics$measurability == "O") / nrow(all_nodes_with_metrics)), 1)`%), or already measured and stored in the current data ecosystem (`r round(100*(sum(all_nodes_with_metrics$measurability == "DW") / nrow(all_nodes_with_metrics)), 1)`%).

We design this experiement as follows. Each supplied model is used to generate synthetic data, assuming each node has an equal effect on its descendants. HOW WILL THIS DO LOOPS? For the "potentially observable" nodes a single descendant node is added, a proxy measurement node that is not the exact node measured but whose value depends only on the potentially observable node and some additional noise. The set of proxy nodes for the potentially observable ($PO$) nodes we denote ($\hat{PO}$). We choose three scenarios of data quality: idyllic, typical, and noisy. Each of these scenarios is represented by the ratio of the path effect to the noise.
...
Here we see that in a "typical" scenario we assume that each path coefficient is `r sim_signal_strength[["typical"]]`. 
...
m_build_model_with_proxy_nodes <- function(m = "S1", x_p_nudge = 0, y_p_nudge = -2,
                                           S.Kn_to_Obs = TRUE) {
    model <- m
...
causal_estimate_over_all_adj_sets <- function(
    ...
    estimate_per_set <- map(adj_sets, function(adj_set) {
        ...
        mean(predict(fit, newdata = data |> mutate({{exposure}} := intervention_val)))
    }
    )
    estimates <- unlist(estimate_per_set)
...
generate_causal_estimate_comparisons <- function(
    ...
    bn_list <- bn_fit_model_list(model_list, b = Beta)
    ...
    estimates <- tibble(
        Model = Model,
        DAG = c("Ground Truth", "Data Warehouse", "Observable", "Potentially Observable") |> 
            factor(levels = c("Ground Truth", "Data Warehouse", "Observable", "Potentially Observable")),
        exposure = exposure, 
        outcome = outcome,
...
all_estimates |>
    ...
    group_by(Model) |> 
    mutate(gt_est_diff = estimate_mean - estimate_mean[DAG == "Ground Truth"] ) |> 
    ungroup() |> 
    ggplot(aes(color = DAG, y = str_c(Model))) + 
    geom_pointinterval(
        aes(x = gt_est_diff, 
            xmin = gt_est_diff - 5 *estimate_sd, 
            xmax = gt_est_diff + 5 *estimate_sd),
           alpha = 0.6, size = 1
    ) +
    geom_pointinterval(
        aes(x = gt_est_diff, 
            xmin = gt_est_diff - 3 *estimate_sd, 
            xmax = gt_est_diff + 3 *estimate_sd),
           alpha = 0.6, size = 3
    ) +
    geom_pointinterval(
        aes(x = gt_est_diff, 
            xmin = gt_est_diff - 1 *estimate_sd, 
            xmax = gt_est_diff + 1 *estimate_sd),
           alpha = 0.6, size = 5
    ) +
    theme_minimal()
