...
- *n* the total number of distinct edges.
- *n_anc_desc* the total number of ancestor - descendant pairs across all graphs. 
- *edge_alignment* is the proportion of edges that were always in the same direction. 
- *descendant_alignment* is the proportion of ancestor - descendant relationships that were always in the same direction.

``` {r}

direct_edge_co_inclusion <- 
    weighted.mean(all_edges_in_models_with_metrics$p_direct,
              all_edges_in_models_with_metrics$N)

causal_path_inclusion <- 
    weighted.mean(all_edges_in_models_with_metrics$p_descendant,
              all_edges_in_models_with_metrics$N)

tb_edge_coherence <- 
    all_edges_sum |> count(p_descendant)
    all_possible_edges_with_metrics |> count(descendant_alignment)
    summarise(
        n_edges = sum(in_graph, na.rm = TRUE),
        n_anc_desc = sum(descendant_in_graph, na.rm = TRUE),
        edge_alignment = mean(
            edge_alignment == "aligned", 
            na.rm = TRUE),
        descendant_alignment = mean(
            descendant_alignment == "descendant:aligned", 
            na.rm = TRUE),
        included_in_a_causal_path = mean()
    ) 
tb_edge_coherence |> 
    knitr::kable("Causal relationship coherence over all models")
```
