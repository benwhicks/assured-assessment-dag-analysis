...
the adjustement sets of some pair of variables into different clusters.
...
plot_vnt_dag <- function(g) {
    ggraph(g, layout = "manual", x = x, y = y)+
        geom_edge_fan(
            ...
            end_cap = circle(6, 'mm')) +
        geom_node_point(aes(label = name, colour = group), size = 6) +
        geom_node_text(aes(label = name), colour = "white", alpha = 0.9) +
        scale_colour_manual(values = first_letter_pal,
                              guide = "none") +
        xlim(c(-0.1,2.1)) +
        ylim(c(-0.1,1.1)) +
        theme_graph()
...
d.L0.C1_C2 <-    gcm_L0_consistency(
    vnt.tdags$Cluster1, 
    vnt.tdags$Cluster2)$d
d.L1.C1_C2 <-    gcm_L1_consistency(
    vnt.tdags$Cluster1, 
    vnt.tdags$Cluster2)$dXY
L2const.C1_C2 <- gcm_L2_consistency(
    vnt.tdags$Cluster1, 
    vnt.tdags$Cluster2, 
    adj_type = "optimal")
d.L2.C1_C2 <- L2const.C1_C2$dXY
r.L2.C1_C2 <- L2const.C1_C2$rXY

# Causal consistency between C1, C1W
d.L0.C1_C1W <-    gcm_L0_consistency(
    vnt.tdags$Cluster1, 
    vnt.tdags$Cluster1W)$d
d.L1.C1_C1W <-     gcm_L1_consistency(
    vnt.tdags$Cluster1, 
    vnt.tdags$Cluster1W)$dXY
L2const.C1_C1W <-  gcm_L2_consistency(
    vnt.tdags$Cluster1, 
    vnt.tdags$Cluster1W, 
    adj_type = "optimal")
d.L2.C1_C1W <- L2const.C1_C1W$dXY
r.L2.C1_C1W <- L2const.C1_C1W$rXY

# Causal consistency between C2, C1W for completeness - probably don't need to include
d.L0.C2_C1W <-    gcm_L0_consistency(
    vnt.tdags$Cluster2, 
    vnt.tdags$Cluster1W)$d
d.L1.C2_C1W <-     gcm_L1_consistency(
    vnt.tdags$Cluster2, 
    vnt.tdags$Cluster1W)$dXY
L2const.C2_C1W <-  gcm_L2_consistency(
    vnt.tdags$Cluster2, 
    vnt.tdags$Cluster1W, 
    adj_type = "optimal")
d.L2.C2_C1W <- L2const.C2_C1W$dXY
r.L2.C2_C1W <- L2const.C2_C1W$rXY


# Causal degradation under coarsening map
deg.L1.FA <- gcm_cluster_L1_degradation(vnt.tdags$Fine1a, first_letter_coarsen)$d_degradation
deg.L1.FB <- gcm_cluster_L1_degradation(vnt.tdags$Fine1b, first_letter_coarsen)$d_degradation
deg.L2.FA <- gcm_cluster_L2_degradation(vnt.tdags$Fine1a, first_letter_coarsen)$d_degradation
deg.L2.FB <- gcm_cluster_L2_degradation(vnt.tdags$Fine1b, first_letter_coarsen)$d_degradation
```
...
- $^\mathcal{L_1}\operatorname{deg}_{\phi}(\mathcal{G}^{[x,z]})$ =
  `r deg.L1.FA`
- $^\mathcal{L_2}\operatorname{deg}_{\phi}(\mathcal{G}^{[x,z]})$ =
  `r deg.L2.FA`
- $^\mathcal{L_1}\operatorname{deg}_{\phi}(\mathcal{G}^{[x]})$ =
  `r deg.L1.FB`
- $^\mathcal{L_2}\operatorname{deg}_{\phi}(\mathcal{G}^{[x]})$ =
  `r deg.L2.FB`
...
```{r}
expand_adj_set <- function(adj_set_vec) {
    adj_set_string <- function(adj_set) {
        if (is.null(adj_set)) {
            return("Not identifiable")
        } else if (length(adj_set) == 0) {
            return("Identifiable without adjustment")
        } else {
            adj_string <- str_c(
                "Identifiable with: ",
                str_c(
    ...
    map(adj_set_vec, adj_set_string) |> 
        as.character()
}

gcm_cluster_L2_degradation(vnt.tdags$Fine1b, first_letter_coarsen)$df |>
    mutate(
        adj_set_fine_model = expand_adj_set(zx_sh),
        adj_set_coarse_model = expand_adj_set(zy_sh)
        ) |> 
    select(x,y, starts_with("adj_set")) |> 
    knitr::kable(caption = "Adjustment set degradation under coarsening of G[x,y]")
...
clean_node_group_cluster_graph <- function(cdag) {
    ...
    if ("measurable" %in% names(cdag |> activate(nodes) |> as_tibble())) {
        dag_out <- 
            dag_out |> 
            mutate(measurability_node_group = coarsen_node_measurability(measurable)) 
    }
    ...
    clustering_L1_degradation <- 
        ...
        imap(
            ...
            tibble(L1_degradation = gcm_cluster_L1_degradation(
                g, code_node_group)$d_degradation
            )) |> 
        list_rbind(names_to = "Model")
...
if (recalculate_distance_matrices) {
    clustering_L2_degradation <- 
        ...
        imap(
            \(g,m)
            tibble(
                L2_degradation = gcm_cluster_L2_degradation(g, code_node_group)$d_degradation
            )) |> 
        list_rbind(names_to = "Model")
...
**average** and **worst-case**. Currently defaulting to the worst-case
scenario.
...
cluster_gcms_from_dist_matrix <- function(D, impute = TRUE, 
                                        clust_method = "ward.D2",
                                        use_worst_case = TRUE) {
    if (any(is.na(D)) & impute) {
        D_imputed <- matrix_fill_NAs_with_colmean(D, include_ids = TRUE)
        idx <- D_imputed$NAs
        D <- D_imputed$M
        message(paste(length(idx)/2, "NA values estimated."))
    } else {
        idx <- NULL
    }
    
    if (use_worst_case) {
        # uses max of diagonals for making symmetric
        D_sym <- pmax(D, t(D))
        clust_method = "complete"
    } else {
        D_sym <- (D + t(D)) / 2
    } 
    hc <- hclust(as.dist(D_sym), method = clust_method)
    hc$D <- D_sym
    hc$imputed <- idx
    return(hc)
}

plotting_Ln_distance_clustering <- function(
        hc, level_label = "L", leg_pos = "none"
        ) {
    D <- hc$D
    nm <- D |> colnames()
    ord <- hc$order
    ...
    p_heat <- ggplot(D_long, aes(to, from, fill = dist)) +
        ...
        scale_fill_viridis_c(
            name = str_c(level_label, "-distance"), 
            direction = -1) +
...
L0_dist <- function(m_i, m_j) {
    L0 <- gcm_L0_consistency(m_i, m_j)
    L0$d
}

if (recalculate_distance_matrices) {
    L0_between_model_metrics <- 
        compute_dist_matrix(CDAGs_node_group, L0_dist, symmetric = TRUE)
    saveRDS(L0_between_model_metrics, "cache/L0_models_dist_matrix.RDS")
} else {
    L0_between_model_metrics <- 
...
```{r L0-clustering-CDAGs}
L0_hclust_models <- cluster_gcms_from_dist_matrix(
    L0_between_model_metrics$D
)
plots_L0_similarities <- plotting_Ln_distance_clustering(
    L0_hclust_models, level_label = "L0", leg_pos = "left")
```

```{r}
plots_L0_similarities$combined.plot
```
...
L1_dist <- function(m_i, m_j) {
  L1 <- gcm_L1_consistency(
      gX = m_i, gY = m_j)
  # if (is.na(L1$dXY)) message("dXY replaced with dYX")
  # coalesce(L1$dXY, L1$dYX)
  L1$d
}
...
if (recalculate_distance_matrices) {
    L1_between_model_metrics <- 
        compute_dist_matrix(CDAGs_node_group, L1_dist, symmetric = TRUE)
    saveRDS(L1_between_model_metrics, "cache/L1_models_dist_matrix.RDS")
} else {
    L1_between_model_metrics <- readRDS("cache/L1_models_dist_matrix.RDS")
}
```

```{r L1-clustering-CDAGs}
L1_hclust_models <- cluster_gcms_from_dist_matrix(L1_between_model_metrics$D)

plots_L1_similarities <- 
    plotting_Ln_distance_clustering(
        L1_hclust_models, level_label = "L1", leg_pos = "left")
```
...
L2_dist <- function(m_i, m_j) {
  L2 <- gcm_L2_consistency(
      gX = m_i, gY = m_j, adj_type = "optimal", causal_pairs_only = FALSE)
  if (is.na(L2$dXY)) message("dXY replaced with dYX")
  d <- coalesce(L2$dXY, L2$dYx) # comment out if not wanted
  return(d)
}

L2_dist_via_projection <- function(m_i, m_j) {
    m_i_latent <- setdiff(gcm_nodelist(m_i), gcm_nodelist(m_j))
    m_j_latent <- setdiff(gcm_nodelist(m_j), gcm_nodelist(m_i))
    M_i <- gcm_latent_projection(m_i, m_i_latent)
    M_j <- gcm_latent_projection(m_j, m_j_latent)
    L2_dist(M_i, M_j)
}

L2_dist_simple <- function(m_i, m_j) {
    v = intersect(
        gcm_nodelist(m_i) |> pull(name),
        gcm_nodelist(m_j) |> pull(name)
    )
    L2_distance_from_adj_fingerprints(
        gcm_adj_fingerprint(m_i),
        gcm_adj_fingerprint(m_j),
        vocab = v
    )
}
...
if (recalculate_distance_matrices) {
    ...
    #     compute_dist_matrix(
    #         CDAGs_node_group, L2_dist, symmetric = FALSE)
    L2_between_model_metrics <- 
        compute_dist_matrix(
            CDAGs_node_group, L2_dist_simple, symmetric = TRUE)
    saveRDS(L2_between_model_metrics, "cache/L2_models_dist_matrix.RDS")
} else {
    L2_between_model_metrics <- readRDS("cache/L2_models_dist_matrix.RDS")
}

```

```{r L2-clustering-CDAGs}
L2_hclust_models <- cluster_gcms_from_dist_matrix(L2_between_model_metrics$D)

plots_L2_similarities <- plotting_Ln_distance_clustering(
    L2_hclust_models,
    level_label = "L2",
    leg_pos = "left"
)
...
L12_hclust_models <- cluster_gcms_from_dist_matrix(
    (L1_between_model_metrics$D + L2_between_model_metrics$D) / 2,
    use_worst_case = T)

plots_L12_similarities_avg_maximal <- 
    plotting_Ln_distance_clustering(L12_hclust_models,
        # method 'average' UPGMA good for non-euclidian
        level_label = "L",
        leg_pos = "none"
    )
```
...
L12_hclust_models |> 
    as.dendrogram() |> 
    highlight_branches(k = 5) |> 
    # highlight_branches_col(rev(magma(100))[9:100]) |> 
    color_labels(k = 5) |> 
    plot()
```

```{r figure--cdag-cluster-dendro-landscape}
L12_hclust_models |> 
    as.dendrogram() |> 
    set("labels_col", 
        value = c("#E69F00", "#E69F00", 
...
L012_hclust_models <- cluster_gcms_from_dist_matrix((L0_between_model_metrics$D +
             L1_between_model_metrics$D + 
             L2_between_model_metrics$D) / 3)

L012_weighted_hclust_models <- cluster_gcms_from_dist_matrix((L0_between_model_metrics$D +
             2 * L1_between_model_metrics$D + 
             3 * L2_between_model_metrics$D) / 6)


plots_L012_similarities_avg_maximal <- 
    plotting_Ln_distance_clustering(
        L012_hclust_models,
        # clust_method = "ward.D2",
        level_label = "L",
...
```{r}
plots_L012_similarities_avg_maximal$combined.plot
# plots_L012_weighted$combined.plot
```

#### Clusters of cluster DAGs

It seems that models M1, M2 (and maybe M4) are close to each other
(GC1), as well as M3, M6 and M7 (GC2) and M5 and M8 (GC3).
...
- Those that did not examine the Misuse of AI (M1, M3)
- Those that did examine the misuse of AI, and within those:
  - Those that mediated effects through student metacognition
    capabilities (M5, M8)
  - Those that did not (M2,M4,M6,M7)

## Latent projection of no-misuse cluster: M1, M3

Key here is they did not examine misuse of AI.

```{r}
plot_latent_clusters_merged(c(1,3), label = T)
p_cdag_c1 <- plot_latent_clusters_merged(c(1,3), label = T) + ggtitle("") +
    coord_equal()+
    theme(panel.border = element_rect(colour = M13_col, fill = NA, linewidth = 1))
ggsave(
...
## Latent projection of AI misuse clusters

```{r}
plot_latent_clusters_merged(c(2,4,5,6,7,8), label = T)
```

Note that the story here mostly lies in the introduced "unmeasured"
confound paths, hence we dive into the two key subgroups of CDAGs.

### Latent projection of main cluster: M2, M4, M6, M7

```{r}
plot_latent_clusters_merged(c(2,4,6, 7), label = T) 
p_cdag_c2a <- plot_latent_clusters_merged(c(2,4,6, 7))  + ggtitle("")+
    coord_equal() +
    theme(panel.border = element_rect(colour = M2467_col, fill = NA, linewidth = 1))
ggsave(
    "figures/minifig--CDAG-clust-2a.png", 
    plot = p_cdag_c2a,
    width = 4, height = 4,
    units = "in")
```

### Latent projection of metacognition cluster: M5, M8

```{r}
# L12 cluster of cluster DAGs
# In cluster strength order
plot_latent_clusters_merged(c(5,8), label = T)
p_cdag_c2a <- plot_latent_clusters_merged(c(5, 8))  + ggtitle("")+
    coord_equal()+
    theme(panel.border = element_rect(colour = M58_col, fill = NA, linewidth = 1))
ggsave(
    "figures/minifig--CDAG-clust-2b.png", 
    plot = p_cdag_c2a,
    width = 4, height = 4,
    units = "in")
```
...
if (recalculate_distance_matrices) {
    L0_between_model_and_simulation_metrics <- 
        compute_dist_matrix(all_CDAGs, L0_dist, symmetric = TRUE)
    saveRDS(L0_between_model_and_simulation_metrics, "cache/L0_dist_matrix.RDS")
} else {
    L0_between_model_and_simulation_metrics <- readRDS("cache/L0_dist_matrix.RDS")
}

L0_hclust_all <- cluster_gcms_from_dist_matrix(L0_between_model_and_simulation_metrics$D)

plots_L0_similarities_full <- plotting_Ln_distance_clustering(
    L0_hclust_all, level_label = "L0")
...
if (recalculate_distance_matrices) {
    L1_between_model_and_simulation_metrics <- 
        compute_dist_matrix(all_CDAGs, L1_dist, symmetric = TRUE)
    saveRDS(L1_between_model_and_simulation_metrics, 
            "cache/L1_dist_matrix.RDS")
} else {
    L1_between_model_and_simulation_metrics <- readRDS("cache/L1_dist_matrix.RDS")
}

L1_hclust_all <- cluster_gcms_from_dist_matrix(L1_between_model_and_simulation_metrics$D)

plots_L1_similarities_full <- plotting_Ln_distance_clustering(
    L1_hclust_all, level_label = "L1")
...
```{r}
plots_L0_similarities_full$combined.plot
```

### L1 consistency between models

Includes the models, $M_k$ and their corresponding DAG learned from
data, $D_k$
...
if (recalculate_distance_matrices) {
    L2_between_model_metrics_full <- 
        compute_dist_matrix(
            ...
            L2_dist, 
            symmetric = FALSE)
    saveRDS(L2_between_model_metrics_full, 
            "cache/L2_dist_matrix.RDS")
} else {
    L2_between_model_metrics_full <- readRDS("cache/L2_dist_matrix.RDS")
}
```

```{r}
L2_hclust_all <- cluster_gcms_from_dist_matrix(L2_between_model_metrics_full$D)

plots_L2_similarities_full <- plotting_Ln_distance_clustering(
    L2_hclust_all, 
    level_label = "L2"
)

plots_L2_similarities_full$combined.plot +
    ggtitle("L2-distance clustering across all models")
```

### Ensemble clustering and comparisons
...
### Averaged L0, L1, L2 similarity between all models

#### Figure -- L-distance clustering across all models

```{r L012-dist-clusters-across-all}

imputed_L1 <- L1_between_model_and_simulation_metrics$D |> 
    matrix_fill_NAs_with_colmean()
imputed_L2 <- L2_between_model_metrics_full$D |> 
    matrix_fill_NAs_with_colmean()

L12_between_model_and_simulation_D_matrix <- 
    (imputed_L1 + imputed_L2) / 2

L12_D_matrix_main_cluster <- 
    L12_between_model_and_simulation_D_matrix[c(2,4,6,7,10,12,14,15), c(2,4,6,7,10,12,14,15)]

L12_D_matrix_MisUse_cluster <- 
    L12_between_model_and_simulation_D_matrix[c(2,4,5,6,7,8,10,12,13,14,15,16), c(2,4,5,6,7,8,10,12,13,14,15,16)]

L12_D_matrix_metacog_cluster <- 
    L12_between_model_and_simulation_D_matrix[c(5,8,13,16), c(5,8,13,16)]

L12_D_matrix_noMisuse_cluster <- 
    L12_between_model_and_simulation_D_matrix[c(1,3,9,11), c(1,3,9,11)]

L012_between_model_and_simulation_D_matrix <- 
    (L0_between_model_and_simulation_metrics$D +
            imputed_L1 + 
                imputed_L2) / 3

L12_hclust_full <- cluster_gcms_from_dist_matrix(
    L12_between_model_and_simulation_D_matrix)
...
L12_hclust_main_cluster <- 
    cluster_gcms_from_dist_matrix(L12_D_matrix_main_cluster)
L12_hclust_MisUse_cluster <- 
    cluster_gcms_from_dist_matrix(L12_D_matrix_MisUse_cluster)
L12_hclust_metacog_cluster <- 
    cluster_gcms_from_dist_matrix(L12_D_matrix_metacog_cluster)
L12_hclust_noMisuse_cluster <- 
    cluster_gcms_from_dist_matrix(L12_D_matrix_noMisuse_cluster)

plots_L12_main_cluster <- 
    plotting_Ln_distance_clustering(
        L12_hclust_main_cluster
    )
```

### Causal consistency within model clusters

The following is from the main cluster:

```{r}
plots_L12_main_cluster$combined.plot
```

This is only excluding the non-AI misuse models:

```{r}
plots_L12_MisUse_cluster <- 
    plotting_Ln_distance_clustering(
        L12_hclust_MisUse_cluster
    )
plots_L12_MisUse_cluster$combined.plot
```

This is the models that examined metacognition:

```{r}
plots_L12_metacog_cluster <- 
    plotting_Ln_distance_clustering(
        L12_hclust_metacog_cluster
    )
plots_L12_metacog_cluster$combined.plot
```

And lastly those that did not explore AI mis-use:

```{r}
plots_L12_noMisuse_cluster <- 
    plotting_Ln_distance_clustering(
        L12_hclust_noMisuse_cluster
    )
plots_L12_noMisuse_cluster$combined.plot
...
- First partition: all except M5, M6 and M8 separated from their learned
  model counterpart.
- Second partition: Only M5 remains with its counterpart D5, and this
  stays close to the end. D5 is L-1 and L-2 consistent with M5.
...
idsMisuseClust <- c(2,4,5,6,7,8,10,12,13,14,15,16)
idsMainClust <- c(2,4,6,7,10,12,14,15)

L_hclust_perfect_learn_misuse_AI_clust <- hclust(
    as.dist(
        d_mat[idsMisuseClust, 
              idsMisuseClust]), 
    method = "complete")

L_hclust_perfect_learn_main_cluster <- hclust(
    as.dist(
        d_mat[idsMainClust, idsMainClust]), 
    method = "complete")

tanglegram(dend1 = as.dendrogram(L_hclust_perfect_learn),
           dend2 = as.dendrogram(L12_hclust_full) |> 
...
tanglegram(dend1 = as.dendrogram(L_hclust_perfect_learn_misuse_AI_clust),
           dend2 = as.dendrogram(L12_hclust_MisUse_cluster) |> 
               highlight_branches_col(),
           main_left = "Target learning", main_right = "L-Consistency",
           sort = TRUE, edge.lwd = 3, lwd = 2,
           common_subtrees_color_lines = TRUE, 
...
tanglegram(dend1 = as.dendrogram(L_hclust_perfect_learn_main_cluster),
           dend2 = as.dendrogram(L12_hclust_main_cluster) |> 
               highlight_branches_col(),
           main_left = "Target learning", main_right = "L-Consistency",
           sort = TRUE, edge.lwd = 3, lwd = 2,
           common_subtrees_color_lines = TRUE, 
...
```{r L012-average-distances-between-all}
