...
Supposing we have some function $\operatorname{d}(x,y) \rightarrow \mathbb{R}$
...
First we define the Jaccard dissimilarity between two sets of statements, with smoothing parameter $\kappa$:

$$\operatorname{d}_{J}(A,B)=\frac{|A \vartriangle B|}{|A\cup B | + \kappa}; \kappa =1$$

Then the causal dissimilarity is defined as: 

$$\mathcal{L}_i\text{-}\operatorname{d}(G,H)=\operatorname{d}_{J}(\Gamma_i(G)\!\upharpoonright\!\mathcal{S}, \ \Gamma_i(H)\!\upharpoonright\!\mathcal{S}); \ \mathcal{S}=\mathbf{V}_G \cap \mathbf{V}_H$$

This is using the restriction
notation, so $\Gamma\!\upharpoonright\!S$ are the statements restricted
to a set S.

#### An alternative framing using projections
...
S, is $\pi_{\mathcal{S}}(G)$ exactly (get ref) - well so some claim. However, that has not proven to be the case with the implementation here. Using the restriction to a set of statements instead of the projection method. 
...
$${\mathcal{L}_i}\text{-}{\operatorname{deg}_{\varphi}}(G)=\operatorname{d}_{J}\big( \varphi \Gamma_{i}(G), \ \Gamma_{i}(\varphi G) \big)$$


#### A discarded definition of causal degradation: 
...
p.vignette.diss <- 
    ...
    (vntPlotC1W +
         geom_richtext(
             ...
             inherit.aes = FALSE) +
         theme(
             panel.border = element_rect(colour = "grey50", linetype = 3,fill=NA, linewidth=1)
         )
    ) +
        ...
        vntPlotC1 + 
            ...
            theme(
                panel.border = element_rect(
                    colour = "grey20", linetype = 1,fill=NA, linewidth=1))
    ) + (
        vntPlotF1A  +
            theme(
                panel.border = element_rect(colour = "grey50", linetype = 3,fill=NA, linewidth=1))
...
code_node_measurability <- function(node) {
    case_when(
        ...
        str_detect(node, "^S\\.Kn") ~ "O", # Forced to O for simulation
        str_detect(node, "^S") ~ "L", 
        # Help
...
``` {r}
p_coarse_show_and_save(7, title = F)
```
...
``` {r}
pg0 <- plot_latent_clusters_merged(
    grouped_cdags$C2a$models, label = T) +
    scale_y_continuous(limits = c(-1, 10))+
    # scale_y_continuous(limits = c(-0.5, 10))+
    theme(
        panel.border = element_rect(
            colour = grouped_cdags$C2a$colour, 
            fill = NA, 
            linewidth = 2)) +
    ggtitle("")
pg0
...
ggsave(
    str_c("figures/projCDAG-labelled-", grouped_cdags$C2a$name,".png"), 
        plot = pg0,
        width = 6, height = 4,
        units = "in")
```
...
Here the categories use the following rules:

- *Observed* node groups are made entirely from nodes in the data warehouse. 
- *Observable* node groups are made from Observable nodes or nodes already in the Data Warehouse. 
- *Likely observable* node groups are made from nodes that are a mix of potentially observable and observable.
- *Potentially observable* node groups are made from nodes that are potentially observable.
- *Likely latent* node groups are made from nodes that are a mix of Latent and Potentially observable nodes. 
- *Latent* node groups are made from nodes that are *Latent*

## Creating a simulated data sets
...
estimate of Student Knowledge on Grade, a close proxy for the generaly problem
of assuring assessment. 
We use the Cluster DAGs as a simplified version with more cohesion between the 
models to faciliate comparison later. 

Several datasets are created:

- Ground truth *simulated data*. This uses every node and the expert elicited GCM to generate a data set. 
- Observed data subset. This uses the *Observed* nodes only from the *simulated data*
- Observable data subset. This uses the *Observed* and *Observable* nodes from the *simulated data*, as well as the *Likely observable* nodes with additional noise added to them. 
- Expansive observable data set. This includes *Observed*, *Observable* and *Likely observable* nodes, as well as the *Potentially observable* nodes with additional noise, representing creating additional data measures to generate proxies for those nodes.


We then apply a structure learning algorithm to each dataset above, except the ground truth.  This represents increasing levels of resourcing applied to
...
of application to discover latent nodes are to be believed then
...
simmed.data <- 
    ...
    map(
        \(d)
        simSEM(tidygraph_to_dagitty(d)))

```

``` {r learn-DAGs-from-simmed-data}
...
bn_fit_simulations <- function(
        ...
        Beta = BETA,
        # Extra nodes, but that have additional noise
        proxy_nodes = NULL, 
        proxy_noise_sd = NULL, # will use Beta /2 if not specified
        blacklist = tibble(
            from = "Grade", 
            to = "S.Kn")) {
    ...
    d.learn <- as.data.frame(data |> select(all_of(observable_nodes)))
    if (!is.null(proxy_nodes)) {
        ...
        if (is.null(proxy_noise_sd)) proxy_noise_sd <- 0.5 * Beta
        d.proxy <- as.data.frame(data |> select(all_of(proxy_nodes)))
        d.proxy <- lapply(d.proxy, \(x) x + rnorm(length(x), 0, proxy_noise_sd))
        d.learn <- bind_cols(d.learn, d.proxy)
    }
    
    # fitting BNet
    learned.fit <- 
        bnlearn::bn.fit(
            bnlearn::hc(d.learn, blacklist = blacklist),
            d.learn)
    learned.DAG <- bn_to_dagitty(learned.fit)

    return( # returns a named list of simmed data and models
        list(
            # Only observable nodes
            observed.data = d.learn,
            learned.fit = learned.fit,
            learned.DAG = learned.DAG
    ))
}

get_nodes_from_measurability_list <- function(g, measure_list) {
    g |> 
        activate(nodes) |> 
            as_tibble() |> 
            filter(measurability_node_group %in% measure_list) |> 
            pull(name)
}

model_name_list <- str_c("M", 1:8) |> set_names()

# Two experiments:
# - Observable, with Likely observable as proxies
# - Observable and Likely observable, with Potentially observable as proxies
fit_observed <- function(M) {
    tDAG <- CDAGs[[M]]
    DAG <- tidygraph_to_dagitty(tDAG)
    bn_fit_simulations(
        dag = DAG,
        data = simmed.data[[M]],
        observable_nodes = tDAG |> 
            get_nodes_from_measurability_list(c("Observed")),
        proxy_nodes = tDAG |> 
            get_nodes_from_measurability_list(c("Observable"))
    )
}

fit_observable <- function(M) {
    tDAG <- CDAGs[[M]]
    DAG <- tidygraph_to_dagitty(tDAG)
    bn_fit_simulations(
        dag = DAG,
        data = simmed.data[[M]],
        observable_nodes = tDAG |> 
            get_nodes_from_measurability_list(c("Observed", "Observable")),
        proxy_nodes = tDAG |> 
            get_nodes_from_measurability_list(c("Likely observable"))
    )
}

fit_potentially_observable <- function(M) {
    tDAG <- CDAGs[[M]]
    DAG <- tidygraph_to_dagitty(tDAG)
    bn_fit_simulations(
        dag = DAG,
        data = simmed.data[[M]],
        observable_nodes = tDAG |> 
            get_nodes_from_measurability_list(c(
                "Observed", 
                "Observable", 
                "Likely observable")),
        proxy_nodes = tDAG |> 
            get_nodes_from_measurability_list(c("Potentially observable"))
    )
}
...
learned_bn_observable <- 
    model_name_list |> 
    map(fit_observable)

learned_bn_potentially_observable <- 
    model_name_list |> 
    map(fit_potentially_observable)
...
## Structure learning with hill climber

``` {r ground-truth-estimate}
est_ground_truth <- 
    model_name_list |> 
    map(
        \(M)
        causal_estimate_over_all_adj_sets(
            dag = tidygraph_to_dagitty(CDAGs[[M]]),
            data = simmed.data[[M]]
        )
    )
```
...
``` {r expert-estimates}
est_expert_observable <- 
    model_name_list |> 
    map(
        \(M)
        causal_estimate_over_all_adj_sets(
            dag = tidygraph_to_dagitty(CDAGs[[M]]),
            data = learned_bn_observable[[M]]$observed.data
        )
    )

est_expert_potentially_observable <- 
    model_name_list |> 
    map(
        \(M)
        causal_estimate_over_all_adj_sets(
            dag = tidygraph_to_dagitty(CDAGs[[M]]),
            data = learned_bn_potentially_observable[[M]]$observed.data
        )
    )

```

``` {r hc-learned-estimates}
est_observed <- 
    model_name_list |> 
    map(
        \(M)
        causal_estimate_over_all_adj_sets(
            dag = learned_bn_observed[[M]]$learned.DAG,
            data = learned_bn_observed[[M]]$observed.data
        )
    )
...
est_observable <- 
    ...
    map(
        \(M)
        causal_estimate_over_all_adj_sets(
            dag = learned_bn_observable[[M]]$learned.DAG,
            data = learned_bn_observable[[M]]$observed.data
        )
    )

est_potentially_observable <- 
    model_name_list |> 
    map(
        \(M)
        causal_estimate_over_all_adj_sets(
            dag = learned_bn_potentially_observable[[M]]$learned.DAG,
            data = learned_bn_potentially_observable[[M]]$observed.data
        )
    )
...
``` {r joining-non-PAG-estimates}

sim_list_to_tibble <- function(sims, sim_name = "") {
    sims |> 
        map(
            \(sim)
            tibble(
                method = sim_name,
                GCM = list(sim$DAG),
                adjustment_sets = list(sim$adjustment_set),
                estimate = mean(sim$estimate),
                se = mean(sim$std_error))
        ) |> 
        list_rbind(names_to = "Model")    
}

sims_observable <- 
    sim_list_to_tibble(est_ground_truth, "Ground truth") |> 
    bind_rows(
        sim_list_to_tibble(est_expert_observable, "Expert GCM")
    ) |> 
    bind_rows(
        sim_list_to_tibble(est_observable, "Learned HC DAG")
    ) |> 
    mutate(data = "Observable")

sims_potentially_observable <- 
    sim_list_to_tibble(est_ground_truth, "Ground truth") |> 
    bind_rows(
        sim_list_to_tibble(est_expert_potentially_observable, "Expert GCM")
    ) |> 
    bind_rows(
        sim_list_to_tibble(est_potentially_observable, "Learned HC DAG")
    ) |> 
    mutate(data = "Potentially observable")
...
fetch_simdata_from_learned_bn <- function(bnlist) {
    bnlist |> 
        map(function(sim) {
            m <- as.matrix(sim$observed.data)
...
var_names_obs <- lapply(sim_list_obs, function(m) {
  nm <- colnames(m)
  if (is.null(nm)) paste0("V", seq_len(ncol(m))) else nm
})
var_names_pot_obs <- lapply(sim_list_pot_obs, function(m) {
  nm <- colnames(m)
  if (is.null(nm)) paste0("V", seq_len(ncol(m))) else nm
})
...
data.frame(
  model   = names(sim_list_obs),
  records = vapply(sim_list_obs, nrow, integer(1)),
  nodes   = vapply(sim_list_obs, ncol, integer(1))
)
```
...
results_obs = learn_many(r.sim_list_obs, alpha=r.alpha, run_fci=True)
results_pot_obs = learn_many(r.sim_list_pot_obs, alpha=r.alpha, run_fci=True)
...
```{r}
#| label: pull-back
res_obs <- py$results_obs
res_obs <- res_obs[names(sim_list_obs)]   # dicts don't guarantee order; restore M1..M8
failed_obs <- vapply(res_obs, function(x) !is.null(x$error), logical(1))
if (any(failed_obs)) {
  message("failed: ", paste(names(res_obs)[failed_obs], collapse = ", "))
  print(vapply(res_obs[failed_obs], function(x) x$error, character(1)))
}

res_pot_obs <- py$results_pot_obs
res_pot_obs <- res_pot_obs[names(sim_list_pot_obs)]
failed_pot_obs <- vapply(res_pot_obs, function(x) !is.null(x$error), logical(1))
if (any(failed_pot_obs)) {
  message("failed: ", paste(names(res_pot_obs)[failed_pot_obs], collapse = ", "))
  print(vapply(res_pot_obs[failed_pot_obs], function(x) x$error, character(1)))
}

adj_icd_obs <- Map(function(x, nm) {
  if (is.null(x$adj_icd)) return(NULL)
  a <- x$adj_icd
  dimnames(a) <- list(nm, nm)
  a
}, res_obs, var_names_obs)

adj_fci_obs <- Map(function(x, nm) {
...
adj_icd_pot_obs <- Map(function(x, nm) {
  if (is.null(x$adj_icd)) return(NULL)
  a <- x$adj_icd
  dimnames(a) <- list(nm, nm)
  a
}, res_pot_obs, var_names_pot_obs)
...
edges_icd_obs <- lapply(adj_icd_obs[!vapply(adj_icd_obs, is.null, logical(1))], pag_edges)
edges_icd_pot_obs <- lapply(adj_icd_pot_obs[!vapply(adj_icd_pot_obs, is.null, logical(1))], pag_edges)
...
``` {r}
pag_to_dagitty <- function(e, boldy = FALSE) {
    # "boldly" treats @-> as ->, taking a guess
    # that this is the correct edge
  ...
  if (boldy) {
      stmts <- mapply(g, e$from, e$to, e$mark_from, e$mark_to, USE.NAMES = FALSE)
  } else {
...
bold_icd_PAGs_obs <- 
    edges_icd_obs |> 
    map(\(x) pag_to_dagitty(x, boldy = TRUE))

bold_icd_PAGs_pot_obs <- 
    edges_icd_pot_obs |> 
    map(\(x) pag_to_dagitty(x, boldy = TRUE))
...
``` {r}
sim_from_pag <- 
    function(Model = "M6", dag_list, data_list) {
    causal_estimate_over_all_adj_sets(
        dag = dag_list[[Model]], 
        data = data_list[[Model]]$observed.data
        ) 
}

sim_from_pag_pot_obs <- 
    function(Model = "M6") {
    causal_estimate_over_all_adj_sets(
        dag = icd_PAGs_pot_obs[[Model]], 
        data = learned_bn_potentially_observable[[Model]]$observed.data
        ) 
}

simulations_ng_PAGs_obs <- 
    model_name_list |> 
    map(
        \(M) 
        sim_from_pag(
            M, 
            dag_list = icd_PAGs_obs,
            data_list = learned_bn_observable))

simulations_ng_PAGs_pot_obs <- 
    model_name_list |> 
    map(
        \(M) 
        sim_from_pag(
            M, 
            dag_list = icd_PAGs_pot_obs,
            data_list = learned_bn_potentially_observable))

simulations_ng_bold_PAGs_obs <- 
    model_name_list |> 
    map(
        \(M) 
        sim_from_pag(
            M, 
            dag_list = bold_icd_PAGs_obs,
            data_list = learned_bn_observable))

simulations_ng_bold_PAGs_pot_obs <- 
    model_name_list |> 
    map(
        \(M) 
        sim_from_pag(
            M, 
            dag_list = bold_icd_PAGs_pot_obs,
            data_list = learned_bn_potentially_observable))
...
ICD_PAG_est_obs <-
    map(
        ...
        \(s)
           tibble(
               method = "Learned ICD PAG",
               data = "Observable",
               GCM = list(s$DAG),
               adjustment_sets = list(s$adjustment_set),
               estimate = mean(s$estimate),
               se = mean(s$std_error)
           )
        ) |> 
    list_rbind(names_to = "Model") 
...
bold_ICD_PAG_est_obs <-
    map(
        simulations_ng_bold_PAGs_obs,
        \(s)
           tibble(
               method = "Overconfident ICD PAG",
               data = "Observable",
               GCM = list(s$DAG),
               adjustment_sets = list(s$adjustment_set),
               estimate = mean(s$estimate),
...
``` {r}
# Dropping bold ones, didn't add much
...
est_to_plot <- 
    all_simulation_estimates |>
    mutate(estimated_ground_truth = is.nan(estimate) & method == "Ground truth") |> 
    mutate(
        estimate = if_else(estimated_ground_truth,
                           ground_truth_estimate_mean_estimate,
                           estimate),
        se = if_else(estimated_ground_truth,
                     ground_truth_error_estimate,
                     se)
    ) |> 
    group_by(Model, data) |> 
    mutate(
        gt_est_diff = 
            estimate - estimate[method == "Ground truth"]) |> 
    ungroup() 
...
est_to_plot |> 
    ...
    mutate(
        y_pos = as.numeric(factor(Model)) + 
            (as.numeric(factor(method)) - mean(as.numeric(factor(method)))) * dodge_amt
    ) |> 
    ggplot(aes(color = method, y = y_pos)) + 
    geom_pointinterval(
        aes(x = estimate,
            xmin = estimate - 1 * se,
            xmax = estimate + 1 * se),
    ...
    annotate("label", x = 0.17, y = c(2.2,3.2,5.2,6.2,8.2), label = "?",
             color = pal_cols[2], alpha = 0.4) +
...
est_to_plot_grouped <- 
    ...
    mutate(model_group = case_when(
        any(estimated_ground_truth) ~ "Unknowable models:\n",
        is.nan(estimate[method == "Expert GCM"]) ~ "Non-identifiable models:\n", 
...
p.est.sims <- 
    ...
    geom_pointinterval(
        aes(x = gt_est_diff,
            xmin = gt_est_diff - 1 * se,
            xmax = gt_est_diff + 1 * se),
           alpha = 0.6, size = 5
    ) +
    scale_y_continuous(
    ...
    # annotate("label", x = -0.05, y = c(2), label = "?",
    #          color = pal_cols[2], alpha = 0.4) +
    # annotate("label", x = -0.025, y = c(3), label = "?",
    #          color = pal_cols[2], alpha = 0.4) +
...
p.est.sims.2 <- 
    ...
    annotate("label", x = -0.0, y = c(1.95), label = "?",
             color = pal_cols[2], alpha = 0.4) +
    annotate("label", x = 0.0, y = c(2.15), label = "?",
             color = pal_cols[4], alpha = 0.4) +
    annotate("label", x = 0.0, y = c(1.15), label = "?",
             color = pal_cols[4], alpha = 0.4) +
    scale_colour_discrete(
        name = "GCM",
        palette = palette.colors()) +
...
ggsave("figures/FIG--simulation-results-no-PAG.png", p.est.sims.3,
       width = 10, height = 3)
...
plot_learned_dag_comparisons <- function(M = "M4",
                                         DATA = "Observable",
                                         ls = 5) {
    models <- 
        all_simulation_estimates |> 
        filter(Model == M, data == DATA,
               method != "Ground truth") |> 
        select(method, GCM)
    
    .fetch_and_add_node_info <- function(g) {
        g |> 
            chuck("GCM", 1) |> 
            ...
            tidygraph::left_join(
                merged_coarse_layout |> 
                    select(name, node_type, x, y), by = "name") |> 
            tidygraph::left_join(
                node_group_nice_name |> 
                    rename(name = node_group), by = "name") |> 
            mutate(name = nice_name) 
    }
    
    g_expert_GCM <- models |> 
        filter(method == "Expert GCM") |> 
        .fetch_and_add_node_info()
        
    
    g_HC_DAG <- models |> 
        filter(method == "Learned HC DAG") |> 
        .fetch_and_add_node_info()
    ...
    obs_node <- gcm_nodelist(g_HC_DAG, as_tibble = FALSE)
    g_proj_expert_GCM <- gcm_projection(
        g_expert_GCM, keep_nodes = obs_node)
    
    g_plotter <- partial(
        gcm_qplot, 
        label_size = ls,
        node_colour_var = "node_type",
        node_colours = node_type_colours)
    
    return(list(
        Expert_GCM.      = g_plotter(g_expert_GCM),
        Expert_proj_GCM  = g_plotter(g_proj_expert_GCM),
        HC_DAG           = g_plotter(g_HC_DAG),
        ICD_PAG          = g_plotter(g_ICD_PAG)
    ))
}

```

## Visualising learning of M4

``` {r}
pM4 <- plot_learned_dag_comparisons("M4", ls = .3)
pM4obs <- plot_learned_dag_comparisons("M6", DATA = "Observable", 
                                       ls = .02)
pM4obs$Expert_GCM +
    scale_x_continuous(expand = c(0.2, 0.2))

ggsave("figures/FIG--M4obs-expert-DAG.png", 
       pM4obs$Expert_GCM +
...
``` {r}
pM4obs$Expert_proj_GCM +
    scale_x_continuous(expand = c(0.1, 0.1))

ggsave("figures/FIG--M4obs-latent-projection.png", 
       pM4obs$Expert_proj_GCM +
    scale_x_continuous(expand = c(0.1, 0.1)),
       width = 5, height = 3.5)
```
...
ggsave("figures/FIG--M4obs-ICD-PAG.png", 
       pM4obs$ICD_PAG +
    scale_x_continuous(expand = c(0.1, 0.1)),
       width = 5, height = 3.5)
```
...
ggsave("figures/FIG--M4obs-HC-DAG.png", pM4obs$HC_DAG +
    scale_x_continuous(expand = c(0.1, 0.1)),
       width = 5, height = 3.5)
```
...
hcDAGs <- 
    ...
    map(
        function(D){
         M <- str_replace(D, "D", "M")
         learned_bn_observable[[M]]$learned.DAG |> 
             dagitty_to_tidygraph()
        })
