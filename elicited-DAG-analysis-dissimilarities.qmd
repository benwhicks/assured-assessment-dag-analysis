...
plot_learned_dag_comparisons <- function(M = "M4",
    ...
    g_proj_expert_GCM <- gcm_projection(g_expert_GCM, keep_nodes = obs_node)
    
    return(list(
        Expert_GCM = gcm_qplot(g_expert_GCM, label_size = ls),
        Expert_proj_GCM = gcm_qplot(g_proj_expert_GCM, label_size = ls),
        HC_DAG = gcm_qplot(g_HC_DAG, label_size = ls),
        ICD_PAG = gcm_qplot(g_ICD_PAG, label_size = ls)
    ))
}
...
pM4obs <- plot_learned_dag_comparisons("M4", DATA = "Observable", ls = 3)
pM4$Expert_GCM +
...
ggsave("figures/FIG--M4obs-latent-projection.png", pM4obs$Expert_proj_GCM +
    scale_x_continuous(expand = c(0.1, 0.1)),
       width = 5, height = 5)
```
