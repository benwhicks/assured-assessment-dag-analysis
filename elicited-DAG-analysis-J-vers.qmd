...
recalculate_distance_matrices <- TRUE
...
if (recalculate_distance_matrices) {
    L1_between_model_dist_matrix <- 
        compute_dist_matrix(CDAGs_node_group,
                            .df = L1_distance_via_latent_projection)
...
## Latent projection of main cluster: M2, M3, M4, M6

```{r}
plot_latent_clusters_merged(c(2,3, 4,6), label = T) 
...
if (recalculate_distance_matrices) {
    # L1 fingerprints
    all_L1_fingerprints <- all_CDAGs |> 
        map(gcm_ci_fingerprint)
    saveRDS(all_L1_fingerprints, 
            "cache/all_L1_fingerprints.RDS")
    # Dist matrix
    all_L1_dist_matrix <- 
        compute_dist_matrix(
            all_CDAGs, 
    ...
    all_L1_fingerprints <- readRDS("cache/all_L1_fingerprints.RDS")
    all_L1_dist_matrix <- readRDS("cache/all_L1_dist_matrix.RDS")
}

L1_hclust_all <- hclust(
    as.dist(all_L1_dist_matrix |> 
...
if (recalculate_distance_matrices) {
    # L1 fingerprints
    all_L2_fingerprints <- all_CDAGs |> 
        map(gcm_adj_fingerprint)
    saveRDS(all_L2_fingerprints, 
            "cache/all_L2_fingerprints.RDS")
    # Dist matrix
    all_L2_dist_matrix <- 
        compute_dist_matrix(
            all_CDAGs,
    ...
    all_L2_fingerprints <- readRDS("cache/all_L2_fingerprints.RDS")
    all_L2_dist_matrix <- readRDS("cache/all_L2_dist_matrix.RDS")
}
```

```{r}
L2_hclust_all <- hclust(
    as.dist(fill_matrix_NAs_with_colmean(all_L2_dist_matrix)),
    method = "ward.D2")
...
all_L012_dist_matrix <- 
    (all_L0_dist_matrix + imputed_L1 + imputed_L2) / 3
